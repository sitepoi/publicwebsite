...
import { isWebsiteCapable, type AppDefinition } from '@/lib/contracts/app-config'
...
export function sitemapObjectTypes(apps: AppDefinition[], siteAppId: string): string[] {
...
export async function collectSiteUrls(
  ...
  // Pages of the website folder (published only). Flow/operation config
  // objects have no slug — they are never URLs. Reserved `default-*`
  // objects (settings/chrome) are infra, not pages.
